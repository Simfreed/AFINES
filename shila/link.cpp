...
{
public:
    Link(double len, actin_ensemble* network, int aindex0, int aindex1, 
            double stiffness, double actin_len, double vis, std::string col)
    {
        //variables that ... what?
        lk              =   stiffness;
        ld              =   len;
...
int main(){
    actin_ensemble net = actin_ensemble(1, 1, 1, 1, 1, 1, 1, 1, 1);
    Link l = Link(1, &net, 1, 2, 1, 1, 1, "0.5");
    return 0;
};
