...
{
public:
    actin_ensemble(double density, double fovx, double fovy, int nx, int ny, double len, double vis)
    {
        view=0.9;
        fov[0]=fovx;
        fov[1]=fovy;
        ...
        av_vel=0;
        visc=vis;
        nrod=int(ceil(density*fov[0]*fov[1]));
        for (int i=0; i<nrod; i++) {
            ld=len;//rng_n(len,1.0);
            theta=rng(0,2*pi);//rng(0,pi/6) + ((double)(rand()%2))*rng(pi,pi+pi/6);
            network.push_back(actin(rng(-0.5*(view*fovx-ld),0.5*(view*fovx-ld)),rng(-0.5*(view*fovy-ld),0.5*(view*fovy-ld)),theta,ld,fov[0],fov[1],nq[0],nq[1],visc));
...
private:
    double fov[2], rho, ld, theta, xnew, ynew, phinew, vpar, omega, vperp, vx, vy, alength, view, a_ends[4], av_vel, visc;
    int nrod, nq[2], xn, yn, qxcm, qycm;
...
int main(int argc, char* argv[])
{
	...
	actin_ensemble net=actin_ensemble(actin_density,xrange,yrange,xgrid,ygrid,actin_length,viscosity);
    std::cout<<"\nAdding motors..";
