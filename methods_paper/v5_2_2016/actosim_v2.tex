...
chain, as depicted in \Cref{fig:t0}B. The linear springs 
...
bending modulus as seen in \Cref{fig:filament}A. A second (\Cref{fig:filament}B)
...
with the subscript $m$ replacing the subscript $cl$ in \Cref{eqn:cl_on,eqn:cl_off,eqn:lever}
...
filament.
\subsubsection{Explanation of \Cref{fig:filament}}
...
($\lambda_2(t)\propto t^{3/4}$). We show  in \Cref{fig:filament}B that our
...
the overdamped limit where the equation of motion is \Cref{eqn:lang} without the
...
These two approximations allow us to rewrite \Cref{eqn:lang} as \Cref{eqn:overdamped} 
